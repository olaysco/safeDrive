...
$val = $db->multipleColumn("SELECT * FROM `files` WHERE `folder_id` LIKE ? AND `presence` = ?", array($dept_id,1));
...
<input type="hidden" name="page" value="Department_files" id="pageName">
    
<h3><i class="fa fa-folder-open"> </i><?php echo ' '.get_dept_name($dept_id); ?> Department</h3>
        <hr class="divider">
    <div class="panel-body">
       <div class="row">
        ...
        <div class="fileItem col-sm-2 context-menu-one">
            ...
            <form method="post" action="inc/download.php" id="gfileDownload">
            ...
            <form method="post" action="inc/decrypt.php" id="gfileDownloadEncrypt">
                <input type="hidden" name="file_loc" id="gfile_loc" value="<?php echo $value['folder_id'].'/'.$value['file_name']?>"/>
                <input type="hidden" name="poll" id="gpoll" value="<?php echo $value['file_pri_key'] ?>"/>
            </form>
